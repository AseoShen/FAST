    ...
    string AESL_num;{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<32> > gmem_pc_buffer(32768);
          int i = 0;
          ...
          if (i > 0) {{
            ...
            ((int*)__xlx_apatb_param_img_in)[j] = gmem_pc_buffer[i].to_int64();
          }
            for (int j = 0, e = 16384; j < e; j += 1, ++i) {
            ((int*)__xlx_apatb_param_img_out)[j] = gmem_pc_buffer[i].to_int64();
          }}}
        } // end transaction
      } // end file is good
...
sc_bv<32> __xlx_tmp_lv = ((int*)__xlx_apatb_param_img_in)[j];
  ...
  __xlx_offset_byte_param_img_out = 16384*4;
  if (__xlx_apatb_param_img_out) {
    for (int j = 0  - 0, e = 16384 - 0; j != e; ++j) {
sc_bv<32> __xlx_tmp_lv = ((int*)__xlx_apatb_param_img_out)[j];
